...
// four are cycle (collect) views; 'confirmations' (pending transactions) and
...
export const WORKLIST_VIEWS = ['overdue', 'due', 'upcoming', 'all', 'confirmations', 'baddebt'] as const
...
// Backend bounds are inclusive (due_date <= due_before, due_date >= due_after),
// and days_overdue = max(0, today - due_date) — so overdue is strictly before
// today, and "due today" is exactly today.
export function viewToParams(view: WorklistView): Partial<WorklistParams> {
  const today = dayjs()
  switch (view) {
    case 'overdue':
      return { unpaid_only: true, due_before: today.subtract(1, 'day').format('YYYY-MM-DD') }
    case 'due':
      return {
        unpaid_only: true,
        due_after: today.format('YYYY-MM-DD'),
        due_before: today.format('YYYY-MM-DD'),
      }
    case 'upcoming':
      return {
        unpaid_only: true,
